import type { Metadata } from "next";
import { Bebas_Neue, Syne, DM_Serif_Display, JetBrains_Mono } from "next/font/google";
import { AdvertiseContent } from "./AdvertiseContent";

const bebas    = Bebas_Neue({ weight: "400", subsets: ["latin"], variable: "--font-bebas", display: "swap" });
const syne     = Syne({ subsets: ["latin"], variable: "--font-syne", display: "swap" });
const dmSerif  = DM_Serif_Display({ weight: "400", style: ["normal", "italic"], subsets: ["latin"], variable: "--font-dm-serif", display: "swap" });
const jbMono   = JetBrains_Mono({ subsets: ["latin"], variable: "--font-jb-mono", display: "swap" });

export async function generateMetadata(): Promise<Metadata> {
  const description =
    "Advertise with Cryptic Daily through transparent display, sponsored, and newsletter placements with full editorial independence.";

  return {
    title: "Advertise",
    description,
    alternates: {
      canonical: "https://crypticdaily.com/advertise",
    },
    robots: { index: true, follow: true },
    openGraph: {
      title: "Advertise | Cryptic Daily",
      description,
      url: "https://crypticdaily.com/advertise",
      images: [{ url: "https://crypticdaily.com/og-default.png" }],
    },
  };
}

export default function AdvertisePage() {
  const fontVars = [bebas.variable, syne.variable, dmSerif.variable, jbMono.variable].join(" ");
  return <AdvertiseContent fontVars={fontVars} />;
}

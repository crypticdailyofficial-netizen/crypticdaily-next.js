...
function Coin({
  ...
  return (
    <div
      className={`cd-coin absolute rounded-full border border-amber-400/15 bg-amber-400/[0.04] backdrop-blur-xl ${className}`}
...
function MiniCard({
  ...
  return (
    <div
      className={`cd-card absolute rounded-2xl border border-amber-400/10 bg-amber-400/[0.03] p-3 backdrop-blur-xl shadow-[0_20px_70px_rgba(146,64,14,0.22)] ${className}`}
      style={{ transformStyle: "preserve-3d" }}
    >
...
export default function AnimatedHero() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.to(state, {
        progress: FINAL_TEXT.length + 1,
        duration: 2,
        ease: "power3.out",
        onUpdate: renderScramble,
        onComplete: () => {
          titleEl.textContent = FINAL_TEXT;
        },
      });
  ...
  return (
    ...
    >
      ...
      <div className="cd-orb absolute left-[7%] top-[16%] h-20 w-20 rounded-full bg-amber-400/12 blur-2xl md:h-28 md:w-28" />
      <div className="cd-orb absolute right-[9%] top-[18%] h-24 w-24 rounded-full bg-orange-400/10 blur-2xl md:h-32 md:w-32" />
      <div className="cd-orb absolute bottom-[8%] left-[22%] h-16 w-16 rounded-full bg-yellow-300/8 blur-2xl md:h-24 md:w-24" />

      <div className="cd-ring absolute left-[11%] top-[18%] hidden h-32 w-32 rounded-full border border-amber-300/10 md:block" />
      <div className="cd-ring absolute bottom-[10%] right-[14%] hidden h-24 w-24 rounded-full border border-amber-300/10 md:block" />
      ...
      <div className="pointer-events-none absolute inset-0 hidden md:block">
        ...
        <MiniCard
          className="left-[19%] bottom-[16%] w-28"
          title="Volume"
          value="$94B"
        />
        <MiniCard
          className="right-[20%] bottom-[18%] w-28"
          title="Pulse"
          value="+8.4%"
        />
      </div>
      ...
      <div className="cd-content relative z-10 flex h-full items-center justify-center px-4">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <p className="mx-auto mt-3 max-w-xl text-xs leading-6 text-amber-50/65 md:text-sm">
            Real-time crypto coverage with a cinematic scrambled headline
            effect.
          </p>
        </div>
      </div>
